...
namespace SixStringSyn.RPGToolkit2D.Editor.Dashboard
{
    ...
    {
        public RPGToolkitDashboardCardData(RPGToolkitAuthoringSection section, IReadOnlyList<RPGToolkitAssetBrowserEntry> entries, int invalidAssetCount, int validationMessageCount, string lastValidationSummary, int questMissingObjectiveCount = 0, int questMissingRewardCount = 0, int dialogueUnreachableNodeWarningCount = 0)
        {
            ...
            DialogueUnreachableNodeWarningCount = dialogueUnreachableNodeWarningCount;
        }
        ...
        public int DialogueUnreachableNodeWarningCount { get; }
        public bool HasWarnings => DuplicateIdCount > 0 || InvalidAssetCount > 0 || QuestMissingObjectiveCount > 0 || QuestMissingRewardCount > 0 || DialogueUnreachableNodeWarningCount > 0 || (Section.WarnWhenEmpty && AssetCount == 0);
    ...
    public static class RPGToolkitAuthoringWorkflow
    {
        ...
        private static readonly IReadOnlyList<RPGToolkitAuthoringSection> _sections = new List<RPGToolkitAuthoringSection>
        {
            ...
            CreateSection("Dialogue", "Build branching conversations and command-driven narrative flow.", typeof(DialogueDefinition), "NewDialogueDefinition.asset", "Tools/RPG Toolkit/Dialogue Graph Editor", EditorToolsDocumentationAnchorPrefix + "dialogue", "Dialogue is most useful when linked from NPCs and backed by conditions or commands.", false, RPGToolkitDashboardCapabilityStatus.Complete, RPGToolkitDashboardCapabilityStatus.Complete, RPGToolkitDashboardCapabilityStatus.Complete, RPGToolkitDashboardCapabilityStatus.Complete, "Dialogue Graph supports node-link authoring, node detail editing, explicit edge/choice editing, search, validation, and dashboard health counts.", global::SixStringSyn.RPGToolkit2D.Editor.DialogueGraph.DialogueGraphEditorWindow.Open),
        ...
        public static RPGToolkitDashboardCardData BuildCardData(RPGToolkitAuthoringSection section, IReadOnlyDictionary<string, string> lastValidationResults = null)
        {
            ...
            var dialogueUnreachableNodeWarningCount = 0;
            ...
            foreach (var entry in entries)
            {
                ...
                if (entry.Asset is DialogueDefinition) dialogueUnreachableNodeWarningCount += result.Messages.Count(message => message.Code == "DIALOGUE_UNREACHABLE_NODE");
            ...
            return new RPGToolkitDashboardCardData(section, entries, invalidAssetCount, validationMessageCount, summary, questMissingObjectiveCount, questMissingRewardCount, dialogueUnreachableNodeWarningCount);
        }
